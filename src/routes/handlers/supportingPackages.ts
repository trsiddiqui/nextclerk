...
class SupportingPackagesHandler {
  public supportingPackageService = new supportingPackageService()

  public createLineItemsSheet = async (
    req: Request,
    res: Response,
    next: NextFunction
  ): Promise<void> => {
    try {
      ...
      const sharedFilePath = await this.supportingPackageService.createLineItemsSheet(
        customerXRefID
      )
...
export default SupportingPackagesHandler
